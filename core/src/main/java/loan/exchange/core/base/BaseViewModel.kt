...
import loan.exchange.core.tools.SingleLiveEvent
import loan.exchange.domain.base.BaseUseCase
import loan.exchange.domain.base.CompletionBlock
import loan.exchange.domain.exceptions.NetworkError
import loan.exchange.domain.exceptions.ServerError
...
open class BaseViewModel<State, Effect> : ViewModel() {
    ...
    protected fun <P, R, U : loan.exchange.domain.base.BaseUseCase<P, R>> U.launch(
        param: P,
        loadingHandle: (Boolean) -> Unit = ::handleLoading,
        block: loan.exchange.domain.base.CompletionBlock<R> = {}
    ) {
        viewModelScope.launch {
            val actualRequest = loan.exchange.domain.base.BaseUseCase.Request<R>().apply(block)

            val proxy: loan.exchange.domain.base.CompletionBlock<R> = {
                onStart = {
                    loadingHandle(true)
    ...
    protected fun <P, R, U : loan.exchange.domain.base.BaseUseCase<P, R>> U.launchNoLoading(
        param: P,
        block: loan.exchange.domain.base.CompletionBlock<R> = {}
    ) {
        viewModelScope.launch {
    ...
    protected fun <P, R, U : loan.exchange.domain.base.BaseUseCase<P, R>> U.launchUnsafe(
        param: P,
        block: loan.exchange.domain.base.CompletionBlock<R> = {}
    ) {
...
suspend fun <P, R> loan.exchange.domain.base.BaseUseCase<P, R>.getWith(param: P): R? {
    var result: R? = null
    val block: loan.exchange.domain.base.CompletionBlock<R> = {
        onSuccess = { result = it }
        onError = { throw it }
    }
    execute(param, block)
